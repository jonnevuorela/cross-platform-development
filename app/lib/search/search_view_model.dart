...
class GithubSearchViewModel extends ChangeNotifier {
  ...
  GithubSearchViewModel({
    required  GithubSearchBloc bloc,
  }) : _bloc = bloc {
  ...
  String get errorMessage => hasError ? (_bloc.state as SearchStateError).error : '';

  List<SearchResultItem> get items => _bloc.state is SearchStateSuccess 
      ? (_bloc.state as SearchStateSuccess).items 
      : [];

  bool get hasNoResults => _bloc.state is SearchStateSuccess 
